...
if (! class_exists('Date')) {
    class Date
    {
        ...
        public static function seconds($step = 1, $start = 0, $end = 60)
        {
            ...
            return Date::range($step, $start, $end);
        }
        ...
        public static function hours($step = 1, $long = false, $start = null)
        {
            ...
            return Date::range($step, $start, $end, true);
        }
        ...
        {
            return Date::range(1, 1, Date::daysInMonth((int) date('M')), true);
        }
        ...
        public static function years($start = 1980, $end = 2024)
        {
            ...
            return Date::range(1, $start, $end, true);
        }
        ...
        {
            $timestamp = ($timestamp == 0) ? getdate() : getdate($timestamp);
         ...
         * @return array
         */
        protected static function range($step, $start, $end, $flag = false)
        {
            $result = array();
            if ($flag) {
         ...
         * @return bool|mixed
         */
        public static function betweendate($day_range, $format = 'd.m.Y', $delimiter = '/')
        {
            $between = false;

            if ($day_range) {
                $day_range_array = explode($delimiter, $day_range);

                if (isset($day_range_array[0]) &&
                    ! empty(trim($day_range_array[0])) &&
                    trim($day_range_array[0]) != ''
                ) {
                    $day_range_array[0] = trim($day_range_array[0]);
                    $day_range_array[1] = isset($day_range_array[1]) ? trim($day_range_array[1]) : null;

                    $between = Date::between($day_range_array[0], $day_range_array[1], $format);
         ...
         * @return mixed
         */
        public static function between($date_min, $date_max, $format = 'd.m.Y')
        {
            $output[0] = self::validate($date_min, $format) ? strtotime($date_min) : false;

            $output[1] = ($output[0] && self::validate($date_max, $format)) ? strtotime($date_max) : false;

            if ($output[1] && $output[0] >= $output[1]) {
                $output[0] = $output[1];
                $output[1] = false;
            }
